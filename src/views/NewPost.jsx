...
const NewPost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const userRes = await fetch(`http://localhost:8000/users/by-username/${username.trim()}`);
    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-gray-950 border border-gray-800 rounded-2xl p-6 shadow-2xl shadow-purple-950/10">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-[11px] uppercase tracking-wider text-gray-400 font-medium mb-2 pl-1">Nombre de Usuario</label>
            <input
              type="text"
              required
              ...
              className="w-full bg-gray-900 border border-gray-800 rounded-xl px-4 py-2.5 text-xs text-gray-200 placeholder-gray-600 focus:outline-none focus:border-purple-500/50 transition-colors"
            />
          </div>
